...
/* -- Default Parameters -- */
float __attribute__((annotate("sensitive"))) mLBolus = 0.500; //default bolus size
float __attribute__((annotate("sensitive"))) mLBigBolus = 1.000; //default large bolus size
float __attribute__((annotate("sensitive"))) mLUsed = 0.0;
int __attribute__((annotate("sensitive"))) mLBolusStepIdx = 3; //0.05 mL increments at first
//float mLBolusStep = mLBolusSteps[mLBolusStepIdx];
float __attribute__((annotate("sensitive"))) mLBolusStep = 0.050;
...
//serial
char __attribute__((annotate("sensitive"))) serialStr[80] = "";
boolean __attribute__((annotate("sensitive"))) serialStrReady = false;
...
void forward_target(int i){
	printf("forward_target executing \n");
	return ;
}
...
int main(int argc, char **args) {
	...
	void (*indirect_func)(LiquidCrystal*, unsigned int, unsigned int);
	indirect_func = &lcd_begin;
	indirect_func(&lcd, 16, 2);
